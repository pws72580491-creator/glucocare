# 글루코케어 Pro (GlucoCare Pro) — v1.7.0
...
unzip ../glucocare-pro-v1_7_0.zip
...
├─ verify.js            의존성 없는 회귀 검증 스크립트 (node verify.js)
...
├─ js/db.js             IndexedDB 저장소 래퍼 + 전체 백업/복원(JSON)
├─ js/foodDb.js         한식 영양 참고 테이블 + 이름 기반 자동 매칭
├─ js/charts.js         캔버스 기반 차트 렌더러 (외부 라이브러리 없음)
├─ js/insights.js       예상 당화혈색소, 목표범위(구간 정의 포함), 식사-혈당 매칭 계산
...
- 식사 사진을 올리면 AI(Gemini Vision)가 사진 속 음식을 각각 인식해서 영양을 추정하고 합산 — 여러 장 추가 가능, 항목별로 잘못 인식된 것만 제거 가능. "📷 촬영" 또는 "🖼 갤러리" 중 선택 — 아래 "사진으로 AI 식단 분석 설정" 참고 (설정 전이거나 실패 시, 기존처럼 음식 이름을 직접 입력하면 자동 전환됨)
- 식사 이름 입력 시 로컬 영양 테이블(`foodDb.js`)에서 자동 매칭 → 탄수화물/단백질/지방/나트륨/GI 자동 채움 (수동 보정 가능)
- 간단한 휴리스틱 기반 "식사 점수"와 식후 혈당 변화(Δ) 표시
- GI·GL 낮음/보통/높음 분류를 나란히 강조 표시 — GL(혈당부하)은 GI × 탄수화물(g) ÷ 100
  (실제 섭취량까지 반영). 식단 입력화면 배지, 기록 목록(GI는 색상 강조·GL은 대표 수치),
  리포트 평균, CSV에 모두 표시됩니다 (새 입력 필드 없이 이미 저장된 GI·탄수화물 값에서 계산)
- 7/30/90일 그래프(혈당·혈압·체중), 목표범위 밴드 표시
- 예상 당화혈색소(eA1C) — ADAG 연구 공식 기반 (참고용, 실제 진단 아님)
- 리포트: 기간 통계 요약, 혈당 분포, 인쇄/PDF 저장(브라우저 인쇄 기능), CSV 내보내기
- 전체 데이터 백업(JSON, 기간 제한 없음) / 백업 파일에서 복원 — "공유" 탭 "데이터 관리"
...
- `/api/analyze-meal`은 인증이 없는 공개 엔드포인트라, 호출마다 Gemini 과금이
  붙습니다. 같은 오리진 확인과 IP당 5분/8회 요청 제한을 걸어뒀지만(서버리스
  인스턴스별 인메모리 방식이라 완벽하진 않습니다), 배포 URL이 널리 노출되거나
  이상 트래픽이 걱정되면 Vercel 함수 호출 수와 Gemini 사용량 대시보드를 가끔
  확인해보세요.
...
- **"공유" 탭 → 전체 데이터 백업(JSON)**: 기간 제한 없이 모든 저장소를 파일 하나로
  내보냅니다. 기기를 바꾸거나 브라우저 데이터가 지워졌을 때 이 파일로 복원하는 게
  유일한 복구 경로입니다 — 아래 CSV/PDF는 진료·기록용이라 되돌려 넣을 수 없습니다.
- **"공유" 탭 → 백업 파일에서 복원**: 위에서 받은 JSON 파일을 선택하면 지금 데이터에
  추가로 복원됩니다(기존 기록은 지워지지 않음). 같은 파일을 두 번 복원하면 중복이
  생기니 확인창이 뜹니다.
- 리포트 탭 → CSV 내보내기: 최근 30일 전체 기록을 CSV로 저장 (스프레드시트·진료 공유용, 가져오기는 지원하지 않음)
